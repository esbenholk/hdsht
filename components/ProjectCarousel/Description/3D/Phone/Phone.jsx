...
export default function Phone({ slice, modal }) {
  const group = useRef();
  const [roatated, setRotated] = useState(false);
  ...
  return (
    <group ref={group} dispose={null}>
      <group scale={1.8} position={[0, 0.4, 0]}>
        <mesh
        ...
        <mesh geometry={nodes.SCREEN.geometry}>
          <meshStandardMaterial color={0x012220} roughness={0} />
          <Html
            prepend
            transform
            distanceFactor={1}
            width={nodes.SCREEN.geometry.width}
            height={nodes.SCREEN.geometry.height}
          >
            <div className={styles.Content} ref={content}>
              {slice?.items?.map((item, index) => {
                return <PrismicRichText field={item.text_block} key={index} />;
              })}
            </div>
          </Html>
        </mesh>
